...
def home_page():
    invalidDomainError = '<!doctype html><html><body><h1>Not a valid domain</h1></body></html>'
    location = request.args.get('l')
    locationHost = getLocationHost(location)
    try:
        isExternal = ipaddress.ip_address(locationHost).is_private

        if not isExternal:
            returnHtml = invalidDomainError
    except:
        try:
            isDomain = validators.domain(locationHost) 
            if isDomain:
